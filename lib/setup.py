...
def setup_init(db_con, db_cur, localcurrency='usd'):
    ...
    try:
        ...
        sql5 = '''create table watchlist (
            
        )
        '''
        
        sql6 = "insert into config_table (id, local_currency) values (?,?)"
        db_cur.execute(sql1)
        db_cur.execute(sql2)
        db_cur.execute(sql3)
        db_cur.execute(sql4)
        db_cur.execute(sql5)
        db_cur.execute(sql6, (1,localcurrency.upper()))
        db_con.commit()
...
def update_schema(db_con, db_cur):
    ''' incremental database schema updater '''
